...
/**
 * Core Steam profile-code verification, keyed by an already-resolved userId.
 * SERVER ONLY. Shared by the web server actions (cookie auth) and the mobile
 * API routes (bearer-token auth) so the logic lives in one place.
 */
...
export async function startSteamVerificationFor(
  userId: string,
  steamInput: string,
): Promise<StartResult> {
  ...
  const ref = parseSteamInput(steamInput);
...
export async function confirmSteamVerificationFor(userId: string): Promise<ConfirmResult> {
  if (!isSteamServerConfigured()) return { ok: false, error: "Steam isn't configured yet." };
  const admin = createAdminClient();
  if (!admin) return { ok: false, error: "Steam isn't configured yet." };
...
export async function unlinkSteamFor(userId: string): Promise<{ ok: boolean; error?: string }> {
  const admin = createAdminClient();
  if (!admin) return { ok: false, error: "Steam isn't configured yet." };
